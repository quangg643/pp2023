...
class Save:
    def compress():
        with open ("students.dat","a") as outfile:
            with open("student.pickle","rb") as infile:
            ...
            with open("course.pick","rb") as infile:
                data_2 = pickle.load(infile)
                outfile.write("\nCourse:\n")
                for course in eval(data_2):
                    outfile.write(f"course id = {course[0]}, course name = {stu[1]}, number of credits = {course[2]}\n")
            with open("studentMarkManagement.pickle","rb") as infile:
                data_3 = pickle.load(infile)
                outfile.write("\nStudentMarkManagement:\n")
                for studentMarkManagement in eval(data_3).items():
                    outfile.write(f"in the {studentMarkManagement[0][1]}, {studentMarkManagement[0][0]}: {studentMarkManagement[1]}\n")
        if os.path.exists("student.pickle"):
            os.remove("students.pickle")
        if os.path.exists("course.pickle"):
            os.remove("course.pickle")
        if os.path.exists("studentMarkManagement.pickle"):
            os.remove("studentMarkManagement.pickle")
